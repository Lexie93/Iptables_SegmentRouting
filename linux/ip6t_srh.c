...
static bool srh_list_match(const struct sk_buff *skb, const struct ipv6_sr_hdr *srh, int srhoff, const struct ip6t_srh2 *srhinfo);
...
static bool srh2_mt6(const struct sk_buff *skb, struct xt_action_param *par)
{
	int hdrlen, psidoff, nsidoff, lsidoff, srhoff = 0;
	const struct ip6t_srh2 *srhinfo = par->matchinfo;
	struct in6_addr *psid, *nsid, *lsid;
	struct in6_addr _psid, _nsid, _lsid;
	struct ipv6_sr_hdr *srh;
	struct ipv6_sr_hdr _srh;
	...
	if (srhinfo->mt_flags & IP6T_SRH_SID_LIST)
		if (NF_SRH_INVF(srhinfo, IP6T_SRH_INV_SID_LIST, !srh_list_match(skb, srh, srhoff, srhinfo)))
			return false;
...
static bool srh_list_match(const struct sk_buff *skb, const struct ipv6_sr_hdr *srh, int srhoff, const struct ip6t_srh2 *srhinfo) 
{
	...
	if (srh->first_segment + 1 < srhinfo->sid_list_size)
		return false;	
	listoff = srhoff + sizeof(struct ipv6_sr_hdr);	
	for (i = 0; i <= (srh->first_segment + 1 - srhinfo->sid_list_size); i++) {
		match = true;
		for (j = 0; j < srhinfo->sid_list_size && match; j++) {
			list_addr = skb_header_pointer(skb, listoff + ((i + j) * sizeof(_list_addr)), sizeof(_list_addr), &_list_addr);	
			if (ipv6_masked_addr_cmp(list_addr, &srhinfo->sid_list_msk[srhinfo->sid_list_size -1 - j],
										&srhinfo->sid_list_addr[srhinfo->sid_list_size -1 - j]))
				match = false;
		}
		if (match)
			return true;
...
static int srh2_mt6_check(const struct xt_mtchk_param *par)
{
	const struct ip6t_srh2 *srhinfo = par->matchinfo;
...
static struct xt_match srh_mt6_reg[] __read_mostly = {
	...
	},
	{
		...
		.family         = NFPROTO_IPV6,
		.match          = srh2_mt6,
		.matchsize      = sizeof(struct ip6t_srh2),
		.checkentry     = srh2_mt6_check,
		.me             = THIS_MODULE,
	}
};
