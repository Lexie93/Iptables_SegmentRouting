...
"[!] --srh-nsid			addr[/mask]	SRH next SID\n"
"[!] --srh-lsid			addr[/mask]	SRH Last SID\n"
"[!] --srh-sid-list			addr[/mask] , ... , addr[/mask] 	List of SRH SID\n" //NEW
);
}
...
static const struct xt_option_entry srh2_opts[] = {
	...
	{ .name = "srh-lsid", .id = O_SRH_LSID, .type = XTTYPE_HOSTMASK,
	.flags = XTOPT_INVERT},
	{ .name = "srh-sid-list", .id = O_SRH_SID_LIST, .type = XTTYPE_STRING, //NEW
	.flags = XTOPT_INVERT},
	{ }
};
#undef s
...
static void srh2_init(struct xt_entry_match *m)
{
	...
	for (__u32 i = 0; i < IP6T_SRH_LIST_MAX_LEN; i++) { //NEW
		memset(srhinfo->sid_list_addr[i].s6_addr, 0, sizeof(srhinfo->sid_list_addr[i].s6_addr));
		memset(srhinfo->sid_list_msk[i].s6_addr, 0, sizeof(srhinfo->sid_list_msk[i].s6_addr));
	}
}
...
static void srh2_parse(struct xt_option_call *cb)
{
	...
	case O_SRH_SID_LIST: //NEW
		...
		for (unsigned int i = 0; i < srhinfo->sid_list_size; i++) {
				memcpy(srhinfo->sid_list_addr + i, sid_list_addr_ + i, sizeof(struct in6_addr));
				memcpy(srhinfo->sid_list_msk + i, sid_list_msk_ + i, sizeof(struct in6_addr));
		}
		free(sid_list_addr_);
		free(sid_list_msk_);		      
		if (cb->invert)
...
static void srh2_print(const void *ip, const struct xt_entry_match *match, int numeric)
{
	...
	if (srhinfo->mt_flags & IP6T_SRH_LSID)
		printf(" lsid %s %s/%u", srhinfo->mt_invflags & IP6T_SRH_INV_LSID ? "!" : "",
			xtables_ip6addr_to_numeric(&srhinfo->lsid_addr),
			xtables_ip6mask_to_cidr(&srhinfo->lsid_msk));
	if (srhinfo->mt_flags & IP6T_SRH_SID_LIST) { //NEW
			printf(" sid-list %s", srhinfo->mt_invflags & IP6T_SRH_INV_SID_LIST ? "!" : "");
			for (unsigned int i = 0; i < srhinfo->sid_list_size; i++) 
				printf(" %s/%u", xtables_ip6addr_to_numeric(srhinfo->sid_list_addr + i),
...
static void srh2_save(const void *ip, const struct xt_entry_match *match)
{
	...
	if (srhinfo->mt_flags & IP6T_SRH_SID_LIST) { //NEW	
		...
		for (int i = 0; i < srhinfo->sid_list_size; i++)
			printf(" %s/%u", xtables_ip6addr_to_numeric(&srhinfo->sid_list_addr[i]),
			xtables_ip6mask_to_cidr(&srhinfo->sid_list_msk[i]));
		}			
}
